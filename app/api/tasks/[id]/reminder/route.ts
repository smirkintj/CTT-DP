...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (task.assignee.notifyOnReminderEmail === false) {
    await createAdminAudit({
      actorId: session.user.id,
      countryCode: task.countryCode,
      message: `Reminder email skipped for "${task.title}" due to assignee preference.`,
      metadata: { action: 'TASK_REMINDER_EMAIL_SKIPPED', taskId: task.id, assigneeEmail: task.assignee.email }
    });
