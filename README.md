...
- Stakeholder onboarding tour (phase 1):
  - lightweight, first-time guided tour on stakeholder dashboard
  - highlights open tasks, filters/search, task cards, and discussions entry
  - includes `Take quick tour` restart action
  - `Don’t show again` preference is stored locally per browser (`localStorage`)
