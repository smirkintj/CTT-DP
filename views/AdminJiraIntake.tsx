...
const CSS = `
  ...
  @keyframes ji-shoot { 0%{left:-10%;top:20%;opacity:0} 10%{opacity:1} 90%{opacity:1} 100%{left:110%;top:70%;opacity:0} }
  @keyframes ji-fadein { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:translateY(0)} }
`;
...
export const AdminJiraIntake: React.FC<AdminJiraIntakeProps> = ({ currentUser, onOpenTask, onCreateTask }) => {
  ...
  const filteredGroups = useMemo(() => {
    ...
    return groups
      ...
      .filter((g) => g.issues.length > 0 || g.error);
  }, [groups, searchTerm]);
  ...
  return (
    <div className="relative overflow-hidden rounded-[28px] border border-slate-800/80 bg-[#060b18] text-white shadow-[0_30px_80px_rgba(2,6,23,0.4)]">
      ...
      <div className="pointer-events-none absolute inset-0 z-0">
        <div className="absolute -top-28 right-0 h-72 w-72 rounded-full bg-indigo-500/12 blur-3xl" style={{ animation: 'ji-drift-a 18s ease-in-out infinite alternate' }} />
        <div className="absolute bottom-0 left-0 h-60 w-60 rounded-full bg-fuchsia-500/10 blur-3xl" style={{ animation: 'ji-drift-b 22s ease-in-out infinite alternate' }} />
        <div className="absolute left-1/3 top-1/3 h-56 w-56 rounded-full bg-rose-500/8 blur-3xl" style={{ animation: 'ji-drift-c 16s ease-in-out infinite alternate' }} />
        <div className="absolute bottom-1/4 right-1/4 h-48 w-48 rounded-full bg-teal-500/8 blur-3xl" style={{ animation: 'ji-drift-a 20s ease-in-out 4s infinite alternate' }} />
      </div>
      ...
      <div className="relative z-10 space-y-8 p-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-slate-500">Admin · UAT Command</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">Ready for Testing</h1>
